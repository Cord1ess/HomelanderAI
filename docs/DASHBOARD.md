...
|---|---|---|---|
| Reference | text (read-only) | (auto) | **Generated by the database** (`HL-<seq>` from `applicants_ref_seq`), see [DATABASE.md §C](DATABASE.md). Fetched as a preview when the backend is up; else "assigned on submit". Never typed |
| Name | text (right col) | **yes** | Client name, operator-entered |
| Phone | text (right col) | **yes** | Operator-entered contact number |
| Date of birth | date (right col) | no | |
| Sex | select (right col) | no | Female / Male / Other / Prefer not to say |
| Face photo | image upload (left col) | **yes** | Single image for **identification only** — shown with the applicant, never run through a model. `.png .jpg`, max 10 MB, preview + remove |

> **Note on names:** this is a deliberate (2026-09-01) reversal of the earlier
> "no name field" rule — see the dated note in [SPEC.md §9](SPEC.md). The name
> and phone are operator-entered for the carrier's use; **evidence** is still
> PII-minimised (file-header identifiers stripped, de-identified copy only).
...
Disabled until: name + phone + face photo filled (1), coverage type and amount
...
one request; the **database assigns the reference** (`HL-<seq>`). On success go
to a confirmation panel that says, in words the operator can read aloud:

> **Evaluation pending.**
> Reference `HL-000012` submitted. Results in **1–2 business days**.
